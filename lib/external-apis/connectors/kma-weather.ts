...
export type KmaWeatherOutput = {
  annualSunlightHours: number | null;
  solarRadiation: number | null;
  prevailingWind: string;
};

type RawItem = {
  sumSsHr?: string; // sunshine duration (hours)
  sumGsr?: string; // solar radiation
  maxWd?: string; // wind direction code
};
...
export function createKmaWeatherConnector(): Connector<
  KmaWeatherInput,
  KmaWeatherOutput
> {
  ...
  return {
    ...
    async execute(input, signal) {
      const apiKey = process.env.KMA_API_KEY || process.env.DATA_GO_KR_API_KEY;
      if (!apiKey) {
        return emptyResult(
          'ASOS 일자료 서비스 활용승인 키가 설정되지 않았습니다.',
        );
      }
      ...
      try {
        ...
        for (const item of items) {
          if (item.sumSsHr) {
            const hours = parseFloat(item.sumSsHr);
            if (!isNaN(hours)) {
              totalSunshine += hours;
              sunshineCount++;
            }
          ...
          if (item.sumGsr) {
            const radiation = parseFloat(item.sumGsr);
            if (!isNaN(radiation)) {
          ...
          if (item.maxWd) {
            windCounts.set(item.maxWd, (windCounts.get(item.maxWd) ?? 0) + 1);
          }
        }
        ...
        const prevailingWind =
          WIND_DIRECTION_MAP[prevailingCode] ?? (prevailingCode || '미확인');

        return {
          data: {
            annualSunlightHours:
              sunshineCount === items.length && items.length >= 365
                ? Math.round(totalSunshine)
                : null,
            solarRadiation:
              radiationCount === items.length && items.length >= 365
                ? Math.round(totalRadiation / 3.6)
                : null,
            prevailingWind,
          },
          ...
          warnings: [
            '최근접 관측소 기반 집계입니다. 일자료 누락 시 연간 합계를 제공하지 않습니다. 최대풍속 풍향의 최빈값이며 연간 주풍향과 다를 수 있습니다.',
          ],
        };
      } catch (error) {
        const message =
