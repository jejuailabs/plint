...
function createScenario(
  input: EnvelopeInput,
  definition: Pick<DevelopmentScenario, 'id' | 'name' | 'strategy'> & {
    coverageFactor: number;
    farFactor: number;
  },
): DevelopmentScenario {
  const buildingCoverageRatio =
    input.buildingCoverageLimit * definition.coverageFactor;
  ...
  const floors = Array.from({ length: floorCount }, (_, index) => ({
    floor: index + 1,
    footprintScale: Math.sqrt(
      Math.min(
        footprintSqm,
        Math.max(0, grossFloorAreaSqm - index * footprintSqm),
      ) / footprintSqm,
    ),
    heightM: FLOOR_HEIGHT_M,
  }));
  const estimatedRevenueKrw = Math.round(
    grossFloorAreaSqm * input.comparablePricePerSqm,
  );
  const estimatedCostKrw = Math.round(
    grossFloorAreaSqm * CONSTRUCTION_COST_PER_SQM +
      input.areaSqm * input.landPricePerSqm * 1.22,
  );
