...
import { isRunpodConfigured, RunpodConfigurationError, submitBlenderModelingJob } from '@/lib/runpod/blender-modeling';
...
const requestSchema = z.object({
  ...
  parcel: z.object({
    ...
    boundary: z.array(z.object({ latitude: z.number(), longitude: z.number() })).max(128).optional(),
  }),
  scenario: z.object({
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return error('INVALID_BODY', parsed.error.issues[0]?.message ?? '모델링 요청 형식을 확인해 주세요.');
    ...
    const { data: analysis } = await supabase
      ...
      .maybeSingle();
    if (!analysis) return error('ANALYSIS_NOT_FOUND', '본인 분석 결과에서만 모델을 생성할 수 있습니다.', 404);
    ...
    return error('RUNPOD_SUBMISSION_FAILED', 'Blender 모델링 작업을 시작하지 못했습니다.', 502);
  }
}
